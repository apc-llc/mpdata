...
{
	printf("Subdomains boundaries computation in scatter/gather\n\n");
	...
	for (int iv = 0; iv < ndomains; iv++)
	{
		...
		size_t szarrays = narrays * (sizeof(char*) +
			domain->grid[0].extsize * sizeof(long));
		for (int i = 0; i < nsubdomains; i++)
			szarrays += narrays * (sizeof(char*) +
				subdomains[i].grid[0].extsize * sizeof(long));
		...
		for (int iarray = 0; iarray < narrays; iarray++)
		{
			...
			pdarrays += domain->grid[0].extsize * sizeof(long);
		}
		for (int i = 0; i < nsubdomains; i++)
		{
			...
			for (int iarray = 0; iarray < narrays; iarray++)
			{
				...
				pdarrays += subdomain->grid[0].extsize * sizeof(long);
			}
	...
	scatter(domains, 0, (char**)arrays1, narrays,
	...
	// Compute 3d cross pattern in subdomains and their edges.
	for (int iv = 0; iv < ndomains; iv++)
	{
		struct domain_t* domain = domains + iv;
		struct domain_t* subdomains = domain->subdomains;
		...
		scatter(subdomains, 0, domain->arrays, narrays,
			sizeof(long), LAYOUT_MODE_CUSTOM);
		...
		gather(subdomains, 0, domain->arrays, narrays,
			sizeof(long), LAYOUT_MODE_CUSTOM);
	}

	// Gather subdomains data to the second array.
	gather(domains, 0, (char**)arrays2, narrays,
		sizeof(long), LAYOUT_MODE_AUTO);
