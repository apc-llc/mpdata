...
void* memcpy_cuda_load(void* dst, const void* src, size_t size)
{
...
void* memcpy_cuda_save(void* dst, const void* src, size_t size)
{
...
#define MIN(a,b) ((a) > (b) ? (b) : (a))
	...
	for (int iv = 0; iv < ndomains; iv++)
		printf("domain %04d nx = %04d, ny = %04d, ns = %04d\n",
			iv, domains[iv].nx, domains[iv].ny, domains[iv].ns);
	...
	{	
		...
		for (int iarray = 0; iarray < narrays; iarray++)
		{
			cudaError_t status = cudaMalloc(
				...
				domains[iv].extsize * sizeof(long));
			assert(status == cudaSuccess);
		}
		
		// Set data copying callbacks.
		domains[iv].load = &memcpy_cuda_load;
		domains[iv].save = &memcpy_cuda_save;
	}

	for (int iv = ngpus; iv < ndomains; iv++)
	{
		domains[iv].arrays = (char**)malloc((sizeof(char*) +
			domains[iv].extsize * sizeof(long)) * narrays);
		char* pdarrays = (char*)(domains[iv].arrays + narrays);
		for (int iarray = 0; iarray < narrays; iarray++)
		{
			domains[iv].arrays[iarray] = pdarrays;
			pdarrays += domains[iv].extsize * sizeof(long);
		}
		
		// Set data copying callbacks.
		domains[iv].load = &memcpy;
		domains[iv].save = &memcpy;
	}

	// Scatter first array data to the subdomains.
	scatter(domains, (char**)arrays1, narrays,
		sizeof(long), LAYOUT_MODE_AUTO);
	...
	for (int iv = 0; iv < ngpus; iv++)
	{
		...
		cross3d_cuda(domain->bx + domain->nx + domain->ex,
			domain->by + domain->ny + domain->ey,
			domain->bs + domain->ns + domain->es,
	...
	for (int iv = ngpus; iv < ndomains; iv++)
	{
		...
		cross3d(domain->bx + domain->nx + domain->ex,
			domain->by + domain->ny + domain->ey,
			domain->bs + domain->ns + domain->es,
			(long*)(domain->arrays[0]), (long*)(domain->arrays[1]));
	}
	...
	gather(domains, (char**)arrays2, narrays,
		sizeof(long), LAYOUT_MODE_AUTO);
